...
const AuthForm = <T extends FieldValues>({ type, schema, defaultValues, onSubmit }: AuthFormProps<T>) => {
  ...
  const handleSubmit: SubmitHandler<T> = async (data: T) => {
    setIsLoading(true);
    try {
      const uri = isSignIn ? `${process.env.NEXT_PUBLIC_API}/user/sign-in` : `${process.env.NEXT_PUBLIC_API}/user/sign-up`;
      ...
      toast.success(isSignIn ? "Signed in successfully" : "Sign up successful", {
        description: isSignIn ? "You have successfully signed in!" : "Your account has been created!",
    ...
    <div className="flex flex-col gap-4">
                ...
                <FormItem>
                  ...
                  <FormControl>
                    {field.name === "universityCard" ? (
                      <FileUpload onFileChange={field.onChange} type="image" accept="image/*" placeholder="Upload your ID" folder="ids" variant="dark" />
                    ) : field.name === "password" ? (
      ...
      {isSignIn && (
        <p className="text-center -mb-3">
          <Link href="/forgot-password" className="text-sm font-normal text-primary hover:text-primary/80 ">
            Forgot password?
          </Link>
        </p>
      )}
      <p className="text-center text-base font-medium">
        ...
        <Link href={isSignIn ? "/sign-up" : "/sign-in"} className="font-bold text-primary hover:text-primary/80 transition-colors">
